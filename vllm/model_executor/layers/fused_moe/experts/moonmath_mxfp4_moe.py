...
the optional package is installed. The oracle offers it ahead of the AITER
class for the same backend, so every other configuration falls through to AITER
unchanged.
"""
...
class MoonmathW4A16SituExperts(AiterW4A16ExpertsMonolithic):
    ...
    def process_weights_after_loading(self, layer) -> None:
        ...
        raw = [
            _aiter_raw(t).view(torch.uint8).transpose(-2, -1)
            for t in (
                layer.w13_weight,
                self.quant_config.w1_precision.weight_scale,
                layer.w2_weight,
                self.quant_config.w2_precision.weight_scale,
            )
        ]
